...
const createBotMessage = (messages) => {
  ...
  messages.forEach((ms, i) => {
    body += `
        <div class="chat bot ${i == 0 ? "first" : ""} ${
      i == messages.length - 1 ? "" : "more"
    }"> ${ms} <span>${new Date().toLocaleTimeString()}</span></div>`;
  });
  return body;
};
...
const sendUser = (message) => {
  ...
  buttons.classList.remove("typing");
  chatBody.innerHTML += messageBody;
  chatBody.innerHTML +=
    '<img src="./Images/loading.gif" alt="loader" id="loader">';
  chatBody.scrollTop = chatBody.scrollHeight;
};
